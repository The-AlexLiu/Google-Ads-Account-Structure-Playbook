# Google Ads Account Structure Playbook
...
- Shopping 或商品表现分析有没有帮助控制商品、毛利和预算；
...
| Standard Shopping / 商品表现分析 | 控制商品、利润、库存、爆品和长尾商品表现 | 0%-20% |
...
如果账户只告诉 Google “有转化”，但没有告诉它“哪种转化更值钱”，Smart Bidding 很容易去找便宜但低质量的订单或线索。对电商来说，当收入、毛利或新客价值能稳定回传时，应该优先评估 value-based bidding，也就是让系统围绕转化价值、毛利或更接近利润的信号学习。

### 2. 出价方式是不是匹配当前阶段

Google Ads 近年的产品方向越来越依赖 Smart Bidding 和转化价值信号。如果转化追踪可靠，系统通常更适合围绕转化或转化价值学习，而不是长期依赖手动调 CPC。Google 官方也建议，当不同转化有不同价值时，应该回传收入或转化价值，并使用 Maximize conversion value 或 Target ROAS。
...
- 电商账户只要订单金额回传可靠，通常应该逐步评估 Maximize Conversion Value / tROAS；
...
Google 官方在 Smart Bidding 相关资料里会鼓励 Broad Match 和自动出价配合使用，也明确说不需要为了优化而按匹配类型过度拆分，因为出价系统会按 query 和 auction-time 信号判断。

但我不会在新账户一开始就盲目全 Broad。

如果转化量不够、否词体系没有搭好、落地页承接不清楚，Broad 很可能先带来一批不相关搜索。我的做法通常是：先用 Phrase / Exact 控制冷启动，用搜索词报告判断真实需求；当转化、否词和落地页都稳定后，再小预算测试 Broad，而不是直接把 Search 交给系统自由扩量。

Search 这里我会特别强调三条底线：

| 底线 | 判断标准 |
|---|---|
| 关键词主题要窄 | 一个 ad group 只解决一类搜索意图，不把品类词、问题词、竞品词混在一起 |
| 搜索词要复盘 | 每周看花费、点击、转化、CVR、CPA、ROAS，持续加否词和扩优质词 |
| 落地页要接得住 | 搜索词、广告文案、商品页内容必须一致，不能承诺一个卖点却落到无关页面 |

Search 不是越自动越好。对我来说，它最重要的价值是可解释：用户搜了什么、广告承诺了什么、落地页接住了什么、最后有没有成交。

在 Search 扩量前，我会先看四个条件：

| 条件 | 没满足时的动作 |
|---|---|
| 搜索词大部分相关 | 先加否词，不扩预算 |
| 高花费词有转化或有效中间行为 | 先停掉明显浪费的词，不急着放 Broad |
| 落地页和搜索意图一致 | 先换页面或重写广告文案 |
| 转化价值能正常回传 | 先修追踪，不切换价值型出价 |

如果这四个条件没过，扩词和加预算只会把问题放大。
...
|---|---|---|
| Search | 标题、描述、落地页文案、附加资产 | 覆盖搜索意图，广告承诺和落地页一致 |
...
但我不会让 PMax 成为账户里唯一能解释增长的 campaign。如果 Search、Feed、商品表现、GA4 和后台数据都解释不了 PMax 的增长，那这个结构还不够健康。

### 7. Demand Gen 是否真的有上层漏斗任务

Demand Gen 适合做视觉种草、场景教育、再激活和扩展需求，尤其是在 YouTube、Shorts、Discover、Gmail 等视觉场景里。

Google 近年持续强化 Performance Max 和 Demand Gen，说明账户结构不能只围绕 Search 设计：Search 捕捉需求，PMax 放大跨渠道转化，Demand Gen 扩展视觉需求。
...
Google Ads 的自动化能力越来越强，但越自动化，越需要先把边界说清楚。否则系统可能确实在“优化”，但优化的是品牌词、老客、低毛利商品、无关页面或者错误转化。

我会先加这些护栏：

| 护栏 | 为什么重要 |
|---|---|
| Brand 和 Non-brand 分开看 | 品牌词 ROAS 通常很高，但它不代表新增需求 |
| Primary conversion 只保留核心目标 | 不要让 Add to Cart、PageView、订阅弹窗这类弱信号影响购买优化 |
| Search 持续加否词 | 扩 Broad 或放大预算前，必须先控制明显无关搜索 |
| PMax 检查 URL expansion | 排除博客、政策页、招聘页、无货页、低转化内容页 |
| 地理位置设置看清楚 | 投目标国家时，要避免流量跑到不该投的地区 |
| 自动应用建议谨慎开启 | Google 的建议不一定符合利润、库存和现金流目标 |
| 品牌词和再营销不要掩盖增长 | PMax ROAS 很高时，要回头看整体 GMV、新客和非品牌表现 |
| 大促单独管理预算和素材 | 不要把 evergreen campaign 改得太重，导致大促后结构失真 |

这些护栏不是为了限制系统，而是为了让自动化在正确范围内发挥作用。
...
| 商品表现 | 能看出哪些商品花费高、哪些商品出单、哪些商品库存或毛利不适合放量 |
...
### 阶段 2：有转化后，逐步引入 Smart Bidding 和受控扩词
...
### 阶段 3：PMax 承担放量角色，但不能失去解释能力

当 Search 和 Feed 已经有一定信号，PMax 可以开始承担重要放量角色。
...
当账户有稳定转化量、商品表现数据、Search 词包和 PMax 学习后，可以进入成熟结构。
...
| Standard Shopping / 商品表现控制 | 控制商品、毛利、长尾和库存 | 0%-20% |
...
- Search：扩关键词、受控测试 Broad、优化广告文案、加否词；
- PMax：补资产、优化 Feed、拆高毛利商品、排除无关 URL；
- Shopping：按商品表现管理爆品、长尾、低毛利、缺货风险；
...
| PMax 某些商品高花费无转化 | 检查商品页、价格、库存、Feed 信息和是否需要从主力投放中排除 |
...
Google 电商投放里，Feed 不是后台资料，而是广告匹配系统的一部分。标题、图片、价格、库存、GTIN 和落地页都会影响表现。
...
如果只开 PMax，但 Feed 很弱，账户会变得难以解释和优化。
...
> 什么时候先控制，什么时候交给自动化，什么时候拆商品，什么时候扩词，什么时候该停下来先修 Feed、追踪、页面或 offer。
...
- `templates/product-feed-performance-review.csv`：商品 Feed 表现复盘模板；
...
- Store Growers：Performance Max Campaigns: The Ultimate Ecommerce Guide。https://www.storegrowers.com/performance-max-campaigns/
- Store Growers：The Ultimate Guide To Google Ads For Ecommerce。https://www.storegrowers.com/google-ads-ecommerce/
- Smarter Ecommerce：How to run Google Shopping alongside Performance Max。https://smarter-ecommerce.com/blog/en/google-shopping/how-to-run-google-shopping-alongside-performance-max-in-2026/
